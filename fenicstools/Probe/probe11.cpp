...
setup_pybind11(cfg)
cfg['sources'] = ['Probe.cpp', 'Probes.cpp', 'StatisticsProbe.cpp', 'StatisticsProbes.cpp']
...
PYBIND11_MODULE(probe11, m)
{
    ...
    py::class_<StatisticsProbe, std::shared_ptr<StatisticsProbe>, Probe>(m, "StatisticsProbe")
        .def(py::init([](const py::array_t<double> x, const py::object v, bool segregated=false){
            auto _v = v.attr("_cpp_object").cast<const FunctionSpace&>();
            const Array<double> _x(x.size(), const_cast<double*>(x.data()));
            return StatisticsProbe(_x, _v, segregated);
        }), py::arg("x"), py::arg("v"), py::arg("segregated")=false)
        .def("eval", [](StatisticsProbe& self, py::object v){
            auto _v = v.attr("_cpp_object").cast<const Function&>();
            self.eval(_v);
        })
        .def("eval", [](StatisticsProbe& self, py::object u, py::object v){
            auto _u = u.attr("_cpp_object").cast<const Function&>();
            auto _v = v.attr("_cpp_object").cast<const Function&>();
            self.eval(_u, _v);
        })
        .def("eval", [](StatisticsProbe& self, py::object u, py::object v, py::object w){
            auto _u = u.attr("_cpp_object").cast<const Function&>();
            auto _v = v.attr("_cpp_object").cast<const Function&>();
            auto _w = w.attr("_cpp_object").cast<const Function&>();
            self.eval(_u, _v, _w);
        })
        .def("erase_snapshot", &StatisticsProbe::erase_snapshot)
        .def("clear", &StatisticsProbe::clear)
        .def("mean", &StatisticsProbe::mean)
        .def("variance", &StatisticsProbe::variance)
        .def("restart_probe", [](StatisticsProbe& self, const py::array_t<double> u, std::size_t num_evals){
            const Array<double> _u(u.size(), const_cast<double*>(u.data()));
            self.restart_probe(_u, num_evals);
        });

    py::class_<StatisticsProbes, std::shared_ptr<StatisticsProbes>, Probes>(m, "StatisticsProbes")
        .def(py::init([](const py::array_t<double> x, const py::object v, bool segregated=false){
            auto _v = v.attr("_cpp_object").cast<const FunctionSpace&>();
            const Array<double> _x(x.size(), const_cast<double*>(x.data()));
            return StatisticsProbes(_x, _v, segregated);
        }), py::arg("x"), py::arg("v"), py::arg("segregated")=false)
        .def("eval", [](StatisticsProbes& self, py::object v){
            auto _v = v.attr("_cpp_object").cast<const Function&>();
            self.eval(_v);
        })
        .def("eval", [](StatisticsProbes& self, py::object u, py::object v){
            auto _u = u.attr("_cpp_object").cast<const Function&>();
            auto _v = v.attr("_cpp_object").cast<const Function&>();
            self.eval(_u, _v);
        })
        .def("eval", [](StatisticsProbes& self, py::object u, py::object v, py::object w){
            auto _u = u.attr("_cpp_object").cast<const Function&>();
            auto _v = v.attr("_cpp_object").cast<const Function&>();
            auto _w = w.attr("_cpp_object").cast<const Function&>();
            self.eval(_u, _v, _w);
        })
        .def("add_positions", [](StatisticsProbes& self, const py::array_t<double> x, const py::object v, bool segregated){
            auto _v = v.attr("_cpp_object").cast<const FunctionSpace&>();
            const Array<double> _x(x.size(), const_cast<double*>(x.data()));
            self.add_positions(_x, _v, segregated);
        })
        .def("get_statisticsprobe", (std::shared_ptr<StatisticsProbe> (StatisticsProbes::*)(std::size_t)) &StatisticsProbes::get_statisticsprobe)
        .def("erase_snapshot", &StatisticsProbes::erase_snapshot)
        .def("clear", &StatisticsProbes::clear)
        .def("restart_probes", [](StatisticsProbes& self, const py::array_t<double> u, std::size_t num_evals){
            const Array<double> _u(u.size(), const_cast<double*>(u.data()));
            self.restart_probes(_u, num_evals);
        })
        .def("set_probes_from_ids", &StatisticsProbes::set_probes_from_ids);
}
